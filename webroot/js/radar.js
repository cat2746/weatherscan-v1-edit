var map, tt, basemap, radarmain, sortedtimestamps, satradsortedtimestamps, satellitemap, interval;
var customMap = false;
...
function loadRadarImages(divID) {
    ...
    .then(data => {
			...
			if (divID == 'radar-1') {sortedtimestamps = sortedtimestampsforfetch};
			sortedtimestampsforfetch.forEach(timestamp =>{
				radardiv.addLayer({
  ...
  interval = setInterval(() => {
    if (i > sortedmaptimestamps.length - 1) {
      clearInterval(interval);
			setTimeout(function() {
				if (divID == 'satrad-1' && loopnum < maxloop) {
